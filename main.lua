...
if library.theme.cursor then
	...
	if success and library.cursor then
		...
		runservice.RenderStepped:Connect(function()
			...
			library.cursor.Visible = userinputservice.MouseEnabled and (userinputservice.MouseIconEnabled or game:GetService("GuiService").MenuIsOpen)
		end)
...
function library:GetGuiParent()
	local safe, gui = pcall(function()
		return coregui
	end)

	return safe and gui or players.LocalPlayer.PlayerGui
end

function library:GetGuiName()
	if library:GetGuiParent() == players.LocalPlayer.PlayerGui then
		return "Chat"
	else
		return "RobloxGui"
	end
...
function library:CreateWindow(name, size, hidebutton)
	...
	window.Main = Instance.new("ScreenGui", library:GetGuiParent())
	window.Main.Name = library:GetGuiName()
			...
			function sector:AddToggle(text, default, callback, flag)
				...
				function toggle:AddSlider(min, default, max, decimals, callback, flag)
					...
					function slider:Refresh()
						local mousepos = game:GetService("Workspace").CurrentCamera:WorldToViewportPoint(mouse.Hit.Position)
...
function library:CreateWatermark(name, position)
	...
	watermark.main = Instance.new("ScreenGui", library:GetGuiParent())
	watermark.main.Name = library:GetGuiName()
